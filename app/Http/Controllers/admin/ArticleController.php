...
class ArticleController extends Controller
{
    ...
    */
    ...
    WHERE horoscopecategory.id = horoscopecate_id limit 1) as horoscopecategory'),'created_at')->where('type',2)->orderBy('id', 'DESC')->get();


    return view('Admin.pages.article.article',compact('data'));
    }
    ...
    */
    ...
    return view('Admin.pages.article.createarticle',compact('horoscopeCategory','tags'));
    }
    ...
    */
    ...
    $data = DB::table('horoscope')->where('title',$request->title)->where('type',2)->first();
    if ($data) {
    return back()->withErrors("This Article already exits")->withInput($request->all());
    }
    ...
    DB::commit();
    return redirect()->route('Article')->withSuccess('Article created successfully');
    ...
    */
    ...
    $horoscopeCategory = DB::table('horoscopecategory')->where('category_type',2)->get();
    return view('Admin.pages.article.edit-article',compact('data','horoscopeCategory'));
    }
    ...
    */
    ...
    $Horoscope->excerpt = $request->excerpt;
    $Horoscope->tags = $request->tags;
    $Horoscope->horoscopecate_id = $request->articlecate_id;
    if($request->hasFile('thumbImage'))
    $Horoscope->thumbImage = $input['thumbImage'];
    $Horoscope->save();
    DB::commit();
    return redirect()->route('Article')->withSuccess('Article updated successfully');
    }catch (\Exception $e){
    DB::rollback();
    return back()->withErrors($e->getMessage())->withInput($request->all());
    }
