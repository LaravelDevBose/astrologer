        ...
        <li class="breadcrumb-item active" aria-current="page">Edit Zordic Article</li>
        </ol>
        </nav>
        </div>
        ...
        <hr>
        <form class="row g-3" method="post" action="{{ route('zordic-article-update') }}">
        @csrf
        ...
        <option <?=$gender->id == $data->zordic_gender  ? 'selected' : ''; ?> value="{{$gender->id}}">{{$gender->gender}}</option>
        @endforeach
        @endif
        </select>
        </div>
        ...
        <option <?=$sign->id == $data->zordicsign_id ? 'selected' : ''; ?> value="{{$sign->id}}">{{$sign->title}}</option>
        @endforeach
        @endif
        </select>
        </div>
