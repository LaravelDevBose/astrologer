		...
		<div class="mx-auto">
			...
			<div class="card border-top border-0 border-4 border-primary">
				...
				<div class="card-body p-5">
					...
					<hr>
					<form class="row g-3" method="post" action="{{route('Horoscope.edit')}}" enctype="multipart/form-data">
						@csrf
						<div class="col-md-12">
						...
						<div class="col-md-4">
							<label for="titleName" class="form-label">Zodiac</label><input type="hidden" name="id" value="{{$data->id}}">
							<select class="form-select mb-3" name="zodiac_id" aria-label="Default select example">
								...
								<option @if($data->zodiac_id==$datas->id) {{'selected="selected"'}}  @endif value="{{$datas->id}}">{{$datas->Name}}</option>
								@endforeach
							</select>
						</div>
						...
						<div class="col-md-4">
							...
							<select class="form-select mb-3" name="horoscopecate_id" aria-label="Default select example">
								...
								<option selected="@if($data->horoscopecate_id==$datas->id) {{'selected'}}  @endif" value="{{$datas->id}}">{{$datas->title}}</option>
								@endforeach
							</select>
						</div>
						...
						<div class="col-md-4">
							...
							<input class="result form-control" type="text" id="date" value="{{$data->horoscopeDate}}" name="horoscopeDate" placeholder="Date Picker...">
						</div>
