...
<div class="page-content">
	...
	<div class="d-flex justify-content-between">
		...
		<!--end breadcrumb-->
		<a class="btn btn-primary px-5" href="{{route('addzordicsign')}}">Add</a>
	</div>

	<div class="row">
		<h6 class="mb-0 text-uppercase">Zodiac</h6>
		<hr />
		@if(session('success'))
					...
					<table id="example2" class="table table-striped table-bordered">
						...
						<tbody>
							...
							<tr>
								<th scope="row">{{$k+1}}</th>
								<td>{{$datas->title}}</td>
								<td>{{$datas->zodiac_symbol}}</td>
								
								<td>{{$datas->mantra}}</td>
								<td>{{$datas->created_at}}</td>
								<td>
								<a href="{{ route('Zordicsign.edit', ['id' => $datas->id]) }}">
                                    <i class="fadeIn animated bx bx-pencil"></i>
                                </a>

									{!! Form::open([
									'method'=>'GET',
									'route' => ['ZordicSign.destroy',$datas->id],
